...
export function Connector({ color, scale = 1 }: { color: string; scale?: number }) {
  ...
  const branch = (rotation: [number, number, number], position: [number, number, number]) => (
    <group rotation={rotation} position={position}>
      <mesh geometry={tube} material={outer} castShadow receiveShadow />
      <mesh geometry={inner} material={innerMaterial} />
      <mesh geometry={rim} material={outer} position={[0, 0.98, 0]} rotation={[Math.PI / 2, 0, 0]} />
      <mesh geometry={rim} material={outer} position={[0, -0.98, 0]} rotation={[Math.PI / 2, 0, 0]} />
    </group>
  );
